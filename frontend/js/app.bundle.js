...
var initApp = (function(app) {
	...
	app.listFilter = function (list, input, anchor) {
		...
		$(input).change( function () {
			...
			if(filter.length > 1) {
				...
				if (listPrev){	
					listPrev.text("Mostrando " + $(list).find('li.js-filter-show').length + " de " + $(list).find('[data-filter-tags]').length);
				}
	...
	app.factoryReset = function () {
		...
		if (typeof bootbox  != 'undefined') {
			...
			bootbox.confirm({
				title: "<i class='fal fa-exclamation-triangle text-warning mr-2'></i> Estas seguro de reiniciar todos los ajustes?",
				message: "<span> Esto pondrá tu menu como inicialmente esta configurado</span>",
				centerVertical: true,
				swapButtonOrder: true,
				buttons: {
					confirm: {
						label: 'Establecer cambios',
