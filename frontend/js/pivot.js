  ...
  callWithJQuery(function($) {
    ...
    aggregators = (function(tpl) {
      return {
        ...
        "Conteo de Valores Unicos": tpl.countUnique(usFmtInt),
        "Lista de Valores Unicos": tpl.listUnique(", "),
        "Suma (decimales)": tpl.sum(usFmt),
        "Suma (entero)": tpl.sum(usFmtInt),
        "Promedio": tpl.average(usFmt),
        "Media": tpl.median(usFmt),
        "Varianza": tpl["var"](1, usFmt),
        "Desviacion Estandar": tpl.stdev(1, usFmt),
    ...
    renderers = {
      ...
      "Tabla con Barras": function(data, opts) {
      ...
      "Mapa de Calor por Columna": function(data, opts) {
    ...
    locales = {
      en: {
        ...
        localeStrings: {
          ...
          selectAll: "Todos",
          selectNone: "Ninguno",
          tooMany: "(demasiados resultados)",
          filterResults: "Filtro por Valor",
          apply: "Aplicar",
          cancel: "Cancelar",
          totals: "Totales",
          vs: "vs",
        ...
        return function(record) {
          ...
          return formatString.replace(/%(.)/g, function(m, p) {
            switch (p) {
              ...
              case "m":
                return zeroPad(date["get" + utc + "Mes"]() + 1);
              case "n":
                return mthNames[date["get" + utc + "Mes"]()];
              case "d":
                return zeroPad(date["get" + utc + "Dia"]());
              case "w":
                return dayNames[date["get" + utc + "Dia"]()];
              case "x":
                return date["get" + utc + "Day"]();
              case "H":
                return zeroPad(date["get" + utc + "Horas"]());
              case "M":
                return zeroPad(date["get" + utc + "Minutos"]());
              case "S":
                return zeroPad(date["get" + utc + "Segundos"]());
              default:
                return "%" + p;
            }
          });
    ...
    pivotTableRenderer = function(pivotData, opts) {
      ...
      result.setAttribute("id", "tabla_combinada");
    ...
    $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
      ...
      try {
        ...
        uiTable = $("<table>", {
          "class": "table pvtUi",
          "id":"data-table-combine"
        }).attr("cellpadding", 5);
        rendererControl = $("<td style='width:250px;'>").html('<b>Tipo de Gráfico:</<b>').addClass("pvtUiCell");
        renderer = $("<select>").addClass('pvtRenderer form-control').html('<option></option>').appendTo(rendererControl).bind("change", function() {
          return refresh();
        });
        ref = opts.renderers;
        for (x in ref) {
        ...
        unused = $("<td>").html("<b>Filtros Disponibles: </b><br><br>").addClass('pvtAxisContainer pvtUnused pvtUiCell');
        ...
        rowOrderArrow = $("<a>", {
          role: "button"
        }).addClass("pvtRowOrder btn btn-primary text-white").data("order", opts.rowOrder).html(ordering[opts.rowOrder].rowSymbol).bind("click", function() {
        ...
        colOrderArrow = $("<a>", {
          role: "button"
        }).addClass("pvtColOrder btn btn-primary hide text-white").data("order", opts.colOrder).html(ordering[opts.colOrder].colSymbol).bind("click", function() {
          $(this).data("order", ordering[$(this).data("order")].next);
          $(this).html(ordering[$(this).data("order")].colSymbol);
          return refresh();
        });
        $("<td style='width:250px;'>").html('<b>Tipo de Conteo:</<b>').addClass('pvtVals pvtUiCell').appendTo(tr1).append(aggregator).append(rowOrderArrow).append(colOrderArrow).append($("<br>"));
        $("<td>").html('<b class="fs-20 text-dorado">Columnas:</b><br><br>').addClass('pvtAxisContainer pvtHorizList pvtCols pvtUiCell bg-primary').appendTo(tr1);
        tr2 = $("<tr>").appendTo(uiTable);
        tr2.append($("<td>").html('<b class="fs-20 text-dorado">Filas: </b><br><br>').addClass('pvtAxisContainer pvtRows pvtUiCell bg-primary').attr("valign", "top"));
